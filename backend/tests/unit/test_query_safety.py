...
# Filterability is decided at reflection time (ColumnInfo.filterable — see
# test_reflection_types_and_models for the per-type policy); _filter_clause
# only enforces it, so a stand-in carrying the flag is all it reads.
FILTERABLE = SimpleNamespace(filterable=True)
OPAQUE = SimpleNamespace(filterable=False)
